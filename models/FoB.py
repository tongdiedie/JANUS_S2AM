...
class FewShotSeg(nn.Module):
    ...
    def _build_prompt_bundle(
        ...
        sam_refine_points = ensure_points_array(
            hard_neg_pts[: int(policy["sam_refine_points"])]
        )
        ...
        return {
            ...
            "k_bg": int(total_bg),
            "shape_complexity": float(compactness), "scale_policy": policy.get("policy_name", "default_sparse"), "sam_mined_points": int(policy["sam_mined_points"]), "sam_mined_min_distance": int(policy["sam_mined_min_distance"]), "sam_mined_avoid_radius": int(policy["sam_mined_avoid_radius"]),
            "janus_flags": {
                "mutual_prompting": bool(self.use_mutual_prompting),
                "hard_background": bool(self.use_hard_background),
                "curvature_allocation": bool(self.use_curvature_allocation), "effective_policy": policy.get("policy_name", "default_sparse"),
            },
        }
