...
def main(_run, _config, _log):
    ...
    if _config["reload_model_path"] is not None:
        model.load_state_dict(
            torch.load(_config["reload_model_path"], map_location="cpu"), strict=False
        )
    sam = SAM(
        ...
        if _config["isic_setting"] == 1:
            ...
            for cat in categories:
                ...
                with torch.no_grad():
                    for _, sample in enumerate(test_loader):
                        ...
                        query_images = [
                            query_image.float().to(device)
                            for query_image in sample["query_images"]
                        ]
            ...
            with torch.no_grad():
                for _, sample in enumerate(test_loader):
                    ...
                    pred = sam(
                        query_images[0][0], prompts, config=_config, return_logits=False
                    )
    ...
    for label_val, label_name in labels.items():
        ...
        support_sample = test_dataset.getSupport(
            label=label_val, all_slices=False, N=_config["n_part"]
        )
        ...
        with torch.no_grad():
            ...
            # Unpack support data.
            support_image = [
                support_sample["image"][[i]].float().to(device)
                for i in range(support_sample["image"].shape[0])
            ]  # n_shot x 3 x H x W, support_image is a list {3X(1, 3, 256, 256)}
            support_fg_mask = [
                support_sample["label"][[i]].float().to(device)
                for i in range(support_sample["image"].shape[0])
            ]  # n_shot x H x W
            ...
            for i, sample in enumerate(test_loader):  # this "for" loops 4 times
                ...
                # Unpack query data.
                query_image = [
                    sample["image"][i].float().to(device)
                    for i in range(sample["image"].shape[0])
                ]  # [C x 3 x H x W] query_image is list {(C x 3 x H x W)}
