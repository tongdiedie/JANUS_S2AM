...
COMMON=(mode=test dataset=isic isic_setting=2 gpu_id=$GPU_ID num_workers=${NUM_WORKERS:-16} n_steps=${N_STEPS:-39001} max_iters_per_load=${MAX_ITERS_PER_LOAD:-3000} eval_fold=$FOLD test_label=[] seed=${SEED:-2025} reload_model_path=$RELOAD_MODEL_PATH sam_checkpoint=$SAM_CKPT encoder_pretrained_weights=$ENCODER_WEIGHTS isic_setting_2_base_path=$DATA_DIR)
